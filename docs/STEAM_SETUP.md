...
2. The current GodotSteam setting is `steam/initialization/app_data/app_id`. Replace `480` with your real App ID for distribution. Keep `steam/initialization/processes/initialize_on_startup=false`; `SteamMessagesTransport` initializes explicitly and runs callbacks every frame.
3. Choose Host Steam to create a friends-only four-seat lobby. The game displays its lobby ID. Friends may join by that ID, through the overlay invitation dialog, or by accepting Steam's join request. Cold starts honor Steam's `+connect_lobby <id>` launch argument. An invitation during an active run presents a leave-and-join choice before changing sessions.
