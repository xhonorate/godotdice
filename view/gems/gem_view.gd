...
func configure(new_gem: Dictionary) -> void:
	...
	gem = new_gem
	var signature := "%s|%s|%d|%d|%d|%s|%s" % [GemMesh.skill_key(gem), str(gem.get("id", "")),
		int(gem.get("carat", 1)), int(gem.get("cut", 0)), int(gem.get("clarity", 3)), str(sealed()), str(gem.get("inclusions", []))]
