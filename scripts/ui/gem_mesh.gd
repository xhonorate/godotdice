extends RefCounted
## Builds a gem as real faceted geometry from its four C's.
##
...
static func flaw_count(clarity: int) -> int:
	## Inclusions frozen in the stone. The top two ranks are clean by definition.
...
static func body_colour(color_key: String, clarity: int) -> Color:
	...
	return hue.lerp(Color("6d7280"), lerpf(Tuning.value("grey_pull"), 0.0, b)).lerp(Color.BLACK, lerpf(0.18, 0.0, b))
...
static func girdle(gem: Dictionary) -> PackedVector2Array:
	...
	return _miscut(_resample(silhouette(Catalog.gem_color(key)), EDGE_STEPS[k - 1]),
		float(CUT_WANDER[k - 1]), int(CUT_CHIPS[k - 1]), key.hash())
...
static func _stone_material(gem: Dictionary, interior: bool) -> StandardMaterial3D:
	...
	material.emission = Color(str(Catalog.GEM_COLORS.get(color_key, Catalog.GEM_COLORS.RED).hex))
...
static func fire_material(gem: Dictionary) -> ShaderMaterial:
	...
	var color_key: String = Catalog.gem_color(Catalog.canonical_key(str(gem.get("key", ""))))
	var b := brilliance(clampi(int(gem.get("clarity", 1)), 1, 5))
	var hue := Color(str(Catalog.GEM_COLORS.get(color_key, Catalog.GEM_COLORS.RED).hex))
	...
	material.set_shader_parameter("tint",
		Vector3.ONE.lerp(Vector3(hue.r, hue.g, hue.b), Tuning.value("fire_tint")))
...
static func etch_material(gem: Dictionary) -> StandardMaterial3D:
	...
	var l: int = clampi(int(gem.get("clarity", 1)), 1, 5)
	var body := body_colour(color_key, l)
	var emblem := GemIcons.emblem(key)
...
static func describe(gem: Dictionary) -> String:
	...
	var lustre: String = ["cloudy, with flaws frozen inside it", "dull, with a visible flaw",
		"clear", "bright and clean", "blazing"][l - 1]
	var shape: String = str(CUTS.get(color_key, "round"))
	return "A %s %s-cut %s stone, %s, %s. Carat %d sets its size, Cut %d how true it was cut, Clarity %d its lustre. Drag to turn it." % [
		size, str(SHAPE_NAMES.get(shape, shape)),
		str(Catalog.GEM_COLORS.get(color_key, {}).get("name", "Red")).to_lower(),
		cut_note(k), lustre, c, k, l]
