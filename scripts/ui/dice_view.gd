...
func configure(new_die: Dictionary, new_roll: Dictionary, is_selected: bool, is_highlighted: bool, tint: Color) -> void:
	...
	accent = tint
	var signature := "%s|%s|%s" % [str(die.get("key", die.get("shape", "D6"))), str(die.get("shape", "D6")), _face_values()]
	if signature != _signature:
		_signature = signature
		_rebuild()
	_face_index = clampi(int(roll.get("face_index", 0)), 0, maxi(0, _frames.size() - 1))
	_target = _orientation(_face_index)
	var token := "%s#%s#%s" % [str(roll.get("die_id", "")), str(roll.get("roll_count", -1)), str(roll.get("face_index", -1))]
	if interactive:
		if not _steered:
			_manual = _target
			_goal = _target
...
func _rebuild() -> void:
	...
	for index in range(_frames.size()):
		...
		label.text = str(_value_at(index))
		label.font_size = 96
