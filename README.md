...
The game opens in your jewellery shop, and every object in it is a menu: the gem sack holds your collection, the jeweller sells a fresh case of gems each day (restocking early costs 1000 gold more each time), the commission board pays for particular stones or deeds and posts the odd special contract, the armor stand sets the three gems each hero carries down the mine, the wall map shows the mines you have opened and the ones beyond them, the door hosts or joins a party, and the mine cart sets out. In a party everyone can browse the shop and ready up at the cart; the host picks the mine. Pick a hero and an unlocked mine, then set out. A mine is a seam of chambers dug one layer at a time with no bottom: your lantern shows the rooms two layers ahead, anything further is a silhouette, and lift beacons show however deep they are. Every step and every noisy room fills the **tremor meter**, faster the deeper you are (fights never move it); when it fills, the mine's boss breaks into the next chamber you enter. An expedition ends when the party rides a lift home, falls, or kills the boss — which also unlocks the mines beyond it.
...
Every hero has **six gem sockets, and each takes one Color of gem**: the first is always Red, the second always Blue, one or two are prismatic and take any Color, and the rest are the hero's own (Ardor: Red, Blue, prismatic, Blue, Green, prismatic; Kait: Red, Blue, Red, Violet, Gold, prismatic; Max: Red, Blue, Red, White, prismatic, Green). A socket is drawn as the cut of the stone it takes, outlined in that Color. Only the first three can be filled at the armor stand; the other three are locked until the expedition begins, for the stones found underground. Between rooms, open Equipment to arrange the sockets, five active dice and three relic slots by dragging between the sockets and the reserve tray (or click an item, then click where it goes; double-click sets or removes one). Strike must remain equipped. A newly appraised, bought or rewarded gem drops into the first open socket of its Color — a prismatic one only if no socket of its own Color is free; a full loadout is never changed for you.

Left of the sockets in a fight sit the hero's **passive** and **signature**, drawn like two more stones. The passive is always working (Stand Firm, Calculated Risk, Second Thought). The signature needs the hardest hand that hero can make and does something enormous, resolving before any gem: Ardor's **Unbreakable Vow** (four of a kind: 20 block to every living hero, then 12 + twice the matched value to every enemy), Kait's **Long Odds** (a total of 33 or more: the total twice to the target, then a stun) and Max's **Master Plan** (a straight of five: 10 + the top of the run to every enemy, and a stun on each). Right-click either card for its sheet.

Requirements are drawn as marks in the manner of Dice Throne rather than as dice showing numbers: two matching dice for a pair, a stair of dice as long as the straight, the sum sign and an arrow for a total, a raised die for a high roll, with the number it needs beside the mark. A gem that fires on every hand shows what it grows with instead — Strike reads "highest die". Ranks are never written as C, K or L: they appear only as their coloured marks, and the combat sockets show just the stone, its name and its requirement. Right-click anything carried over a combatant's head to see what it does. Fights end on the battlefield: the victory (or defeat) is announced there and the spoils are turned over one by one before the party moves on. The journal explains every rule and content definition.
...
- Three heroes with their specified HP, starting dice, three starting gems, six Coloured sockets, a passive trait and a signature.
...
| `scripts/ui/hub.gd`, `scripts/ui/hub_screens.gd`, `assets/hub/` | The shop: clickable SVG objects and the screens they open |
| `scripts/ui/seam_map.gd`, `scripts/ui/appraisal_table.gd` | The seam as a map, and the 3D table the haul is judged on |
