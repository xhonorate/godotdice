...
- **The Descent**: a shaft charted one stretch at a time. Between landings the way down is a lattice of chambers whose tunnels split and rejoin; the lantern shows two depths ahead, past that only glints, and ore lights the whole stretch. Chambers are fights, elites, veins, oddities (gambles with your stones and dice: re-cut, tumble a stone into another skill, hammer a die a size bigger, trade dice with a tinker) and merchants (stones and dice for ore, a buyer for appraised stones, and appraisals that cost more each time at the same stall); every stretch holds at least one merchant. A landing every fourth depth: each player takes one respite (rest for 30% of their health, one free appraisal, or polish a stone a Cut step truer), then the party votes up the lift or on down. A Warden guards 8, 16 and 24. Below 24 the mine is Endless.
- **The bench** is open whenever no fight is on: from the Bench button, the haul count, or B. Gems on one tab (the rail above, the haul below), dice on the other (the tray above, the bag below); drag to set, swap and take out, or click for the same as buttons.
