...
## Bot estimates hands from the real resolver and uses only public commands.
## This is a smoke sample, not a balance or win-rate claim.
...
func run() -> void:
	...
	for profile in ["short_9","expedition_18"]:
		for hero_id in ["ARDOR","KAIT","MAX"]:
			for seed_value in ([101,202,303] if profile == "short_9" else [101]):
				var engine: RefCounted = _fresh(hero_id,seed_value,false,profile)
				for _step in range(1000):
					if engine.state.phase == "summary": break
					if not _step_bot(engine): break
				var state: Dictionary = engine.state
				if state.phase != "summary": errors.append("Bot did not reach summary: "+profile+" "+hero_id+" seed "+str(seed_value)+" phase "+str(state.phase))
				var result: Dictionary = {"hero":hero_id,"seed":seed_value,"profile":profile,"outcome":state.get("outcome",""),"room":state.room_index,"turns":state.statistics.turns,"gold_earned":state.statistics.gold_earned,"remaining_hp":state.heroes[0].hp}
...
func _fresh(hero_id: String, seed_value: int, save: bool = false, profile: String = "short_9") -> RefCounted:
	...
	var result: Dictionary = engine.new_run({"heroes":[{"id":"bot","hero_id":hero_id}],"profile":profile,"seed":seed_value,"run_id":"smoke-"+profile+"-"+hero_id+"-"+str(seed_value),"autosave":save})
		...
		"support":
			match str(state.room.kind):
				"shop":
					...
					for offer in offers:
						if not offer.claimed and unit.gold >= offer.price and _upgrade_value(unit,offer.gem) > 0.5:
				...
				"lapidary":
					...
					for item in unit.gems:
						...
						for property in ["cut","clarity"]:
							if item[property] >= 5 or unit.gold < (item[property]+1)*5: continue
...
func _small_state(state: Dictionary) -> Dictionary:
	return {"heroes":state.heroes.duplicate(true),"enemies":state.enemies.duplicate(true),"turn":state.turn,"act":state.act,"party_size":state.party_size,"battle_outcome":""}
...
func _autosave_regression() -> void:
	...
	if not _command(engine,"VoteRoom",{"offer_id":engine.state.offers[0].id}): return
	...
	if not _command(engine,"ChooseReward",{"kind":"gem","offer_id":""}): return
