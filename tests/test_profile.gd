...
func _test_return() -> void:
	...
	var sockets_before: int = profile.heroes[hero_key].loadout.size()
	var kept_venom: Dictionary = Profile.decide_return_gem(profile, "h3", true)
	check(kept_venom.socketed == hero_key and "VENOM" in profile.heroes[hero_key].loadout and profile.heroes[hero_key].loadout.size() == sockets_before + 1, "A new gem drops into the played hero's open socket")
