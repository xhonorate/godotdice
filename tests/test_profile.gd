...
func _test_loadouts() -> void:
	...
	check(not Profile.set_loadout(profile, "ARDOR", ["STRIKE", "BLOCK", "SUNDER", "ARC_BURST"]).is_empty(), "More than the three carried sockets is refused")
	check(not Profile.set_loadout(profile, "ARDOR", ["STRIKE", "SUNDER"]).is_empty(), "A Red gem does not fit the Blue second socket")
	check(not Profile.set_loadout(profile, "ARDOR", ["BLOCK", "STRIKE"]).is_empty(), "A Blue gem does not fit the Red first socket")
	check(Profile.set_loadout(profile, "ARDOR", ["STRIKE", "", "SUNDER"]).is_empty(), "A socket may be left open, and a prismatic socket takes a Red gem")
	check(profile.heroes.ARDOR.loadout == ["STRIKE", "", "SUNDER"], "A loadout is stored one entry per carried socket")
	...
	check(int(gems[0].socket) == 0 and int(gems[1].socket) == 2, "Loadout gems keep the socket they were set in")
	check(int(gems[0].carat) == 2 and gems[0].id != gems[1].id, "Loadout gems carry collection ranks and distinct IDs")
	## An older profile saved its loadout as a plain list; it is set into sockets it fits.
	var legacy: Dictionary = profile.duplicate(true)
	legacy.heroes.ARDOR.loadout = ["BLOCK", "STRIKE", "SUNDER", "INTERPOSE"]
	var migrated: Dictionary = Profile.normalize(legacy)
	check(migrated.ok and migrated.profile.heroes.ARDOR.loadout.size() == Catalog.LOADOUT_CARRY, "A plain-list loadout becomes one entry per carried socket")
	check(migrated.profile.heroes.ARDOR.loadout[0] == "STRIKE" and migrated.profile.heroes.ARDOR.loadout[1] == "BLOCK", "Migrated gems land in sockets of their own Color")
...
func _test_return() -> void:
	...
	check(kept.socketed.is_empty(), "A kept gem the loadout already carries takes no second socket")
	var hero_key: String = str(profile.selected_hero)
	var carried: Array = profile.heroes[hero_key].loadout.duplicate()
	carried[2] = ""
	check(Profile.set_loadout(profile, hero_key, carried).is_empty(), "A carried socket can be emptied")
	var kept_venom: Dictionary = Profile.decide_return_gem(profile, "h3", true)
	check(kept_venom.socketed == hero_key and profile.heroes[hero_key].loadout[2] == "VENOM", "A new gem drops into the played hero's open socket its Color fits")
	...
	check(finished.sold.is_empty() and profile.pending_return.is_empty(), "Leaving a judged table sells nothing more")
	check(int(profile.lifetime.gems_kept) == 2 and int(profile.lifetime.gems_sold) == 1 and int(profile.lifetime.runs) == 1, "Lifetime counts runs, keeps and sales")
