...
func _test_workshop_oddities(rng: RandomNumberGenerator, ctx: Dictionary) -> void:
	...
	check(filed.ok and str(p.dice[2].shape) == "D12" and str(p.dice[2].id) == "d20" and p.dice[2].faces.size() == 12 and filed.dice.size() == 1, "the smithy files a d20 down to a d12 that keeps its id")
	check(DeepOddities.apply({"kind": "downsize"}, p, {"die_id": "d4"}, rng, ctx).ok and str(p.dice[0].shape) == "D4", "a hammered die files back down")
	check(not DeepOddities.apply({"kind": "downsize"}, p, {"die_id": "d4"}, rng, ctx).ok, "a d4 is as small as dice come")
	...
	check(not DeepOddities.resize_refusal(die_of.call("D20", "big"), 1).is_empty() and DeepOddities.resize_refusal(die_of.call("D20", "big"), -5).is_empty(), "a d20 goes down five sizes and up none")
